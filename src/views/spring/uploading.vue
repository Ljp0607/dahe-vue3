...
import { getTypeNo } from "@/api/spring/index";
...
const getParentTypeNo = () => {
  getTypeNo(route.query.drawRule as string).then(
    (
      res: Array<{
...
function onConfirm(e: any) {
  ...
  Info.location_address = data.options[e.selectedIndexes].type_name;
  Info.creator_type = data.options[e.selectedIndexes].type_no;
  Info.creatorType = data.options[e.selectedIndexes].type_no;
...
.container {
  ...
  header {
    width: 750px;
    min-height: 664px;
    background: url("https://imgcdn.dahebao.cn/20230118/20230118111411943325.png");
